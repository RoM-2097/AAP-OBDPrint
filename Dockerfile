FROM mcr.microsoft.com/dotnet/aspnet:10.0-nanoserver-ltsc2022 AS base
WORKDIR /app
EXPOSE 5087

ENV ASPNETCORE_URLS=http://+:5087

FROM mcr.microsoft.com/dotnet/sdk:10.0-nanoserver-ltsc2022 AS build
ARG configuration=Release
WORKDIR /src
COPY ["AAP-OBDPrint.csproj", "AAP-OBDPrint/"]
RUN dotnet restore "AAP-OBDPrint\AAP-OBDPrint.csproj"
COPY . .
WORKDIR "/src/AAP-OBDPrint"
RUN dotnet build "AAP-OBDPrint/AAP-OBDPrint.csproj" -c $configuration -o /app/build

FROM build AS publish
ARG configuration=Release
RUN dotnet publish "AAP-OBDPrint/AAP-OBDPrint.csproj" -c $configuration -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "AAP-OBDPrint/AAP-OBDPrint.dll"]
